...
func (p *Processor) DownloadClip(ctx context.Context, id, clipURL string) (file, thumb string, dur int, err error) {
	...
	return videoRel(id), p.makeThumb(cctx, id), p.probeDuration(cctx, out), nil
}

// SaveUpload сохраняет загруженный видео-файл, делает превью и узнаёт длительность.
func (p *Processor) SaveUpload(ctx context.Context, id string, src io.Reader) (file, thumb string, dur int, err error) {
	out := p.abs(videoRel(id))
	f, e := os.Create(out)
	if e != nil {
		return "", "", 0, e
	...
	return videoRel(id), p.makeThumb(ctx, id), p.probeDuration(ctx, out), nil
}
...
// Remove удаляет файлы хайлайта (видео и превью) — при удалении/отклонении.
func (p *Processor) Remove(file, thumb string) {
	if file != "" {
		_ = os.Remove(p.abs(file))
	}
	if thumb != "" {
		_ = os.Remove(p.abs(thumb))
	}
}
